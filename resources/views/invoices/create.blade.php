                    ...
                    <form method="POST" action="{{ route('invoices.store') }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-input id="invoice_number" class="block mt-1 w-full" type="text" name="invoice_number" :value="old('invoice_number', 'INV-' . date('Ymd') . '-' . rand(1000, 9999))" required />
                            </div>

                            <div>
                                <x-label for="client_name" :value="__('Client Name')" />
                                <x-input id="client_name" class="block mt-1 w-full" type="text" name="client_name" :value="old('client_name')" required />
                            </div>

                            <div>
                                <x-label for="client_email" :value="__('Client Email')" />
                                <x-input id="client_email" class="block mt-1 w-full" type="email" name="client_email" :value="old('client_email')" />
                            </div>

                            <div>
                                <x-label for="issue_date" :value="__('Issue Date')" />
                                <x-input id="issue_date" class="block mt-1 w-full" type="date" name="issue_date" :value="old('issue_date', date('Y-m-d'))" required />
                            </div>

                            <div>
                                <x-label for="due_date" :value="__('Due Date')" />
                                <x-input id="due_date" class="block mt-1 w-full" type="date" name="due_date" :value="old('due_date', date('Y-m-d', strtotime('+30 days')))" required />
                            </div>
